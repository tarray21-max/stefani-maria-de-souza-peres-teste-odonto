...
export function useServiceMatrixItems(clientId: string | null) {
  ...
  const refresh = useCallback(async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select("id,name,area,is_default,default_key,disabled,position")
  ...
  const items = useMemo(() => {
    ...
    const custom = rows
      ...
      .map((r): ServiceMatrixItem => ({ id: r.id, name: r.name, area: r.area, isDefault: false, defaultKey: null, position: r.position }));
    return [...mergedDefaults, ...custom].sort((a, b) => a.position - b.position || a.name.localeCompare(b.name, "pt-BR"));
  }, [rows]);

  const addItem = useCallback(async (name: string, area: ServiceArea) => {
  ...
  const updateItem = useCallback(async (item: ServiceMatrixItem, name: string, area: ServiceArea) => {
    if (!clientId) throw new Error("Cadastre uma clínica.");
    const payload = item.isDefault
      ? { id: item.id.startsWith("default:") ? undefined : item.id, client_id: clientId, name, area, is_default: true, default_key: item.defaultKey, disabled: false, position: item.position }
      : { id: item.id, client_id: clientId, name, area, is_default: false, default_key: null, disabled: false, position: item.position };
