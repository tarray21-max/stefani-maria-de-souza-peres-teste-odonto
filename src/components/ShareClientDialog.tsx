...
import { useClientShare, type MemberRole } from "@/lib/use-invitations";
...
export function ShareClientDialog({ open, onOpenChange, clientId, clientName }: Props) {
  const { user } = useAuth();
  const { members, invitations, invite, cancelInvitation, removeMember, changeRole } = useClientShare(clientId);
  ...
  const send = async () => {
    ...
    setEmail("");
    toast.success("Convite criado. Será aceito assim que o usuário entrar.");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Compartilhar {clientName ?? "clínica"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Membros</div>
            <div className="space-y-1">
              {members.length === 0 && <p className="text-xs text-muted-foreground">Apenas você.</p>}
              {members.map((m) => (
                <div key={m.id} className="flex items-center gap-2 p-2 rounded hover:bg-muted/60">
                  <UserCheck className="w-4 h-4 text-muted-foreground" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm truncate">{m.display_name || m.email || m.user_id.slice(0, 8)}</div>
